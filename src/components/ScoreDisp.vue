<template>
  <div class="ScoreDisp">
    ...
    <table id="player1" border=1>
      ...
      <tr>
        <td>{{getScoreForm.player1.name}}</td>
        <td>{{getScoreForm.player1.ball}}/{{getScoreForm.player1.goal}}</td>
        <td>{{getScoreForm.player1.safety}}</td>
        <td>{{getScoreForm.player1.ace}}</td>
        <td>{{getScoreForm.player1.breakrun}}</td>
      </tr>
    </table>
    <table id="common" border=1>
      ...
      <tr>
        <td>{{getScoreForm.inning}}</td><td>{{getScoreForm.dead}}</td>
      </tr>
    </table>
    <table id="player2" border=1>
      ...
      <tr>
        <td>{{getScoreForm.player2.name}}</td>
        <td>{{getScoreForm.player2.ball}}/{{getScoreForm.player2.goal}}</td>
        <td>{{getScoreForm.player2.safety}}</td>
        <td>{{getScoreForm.player2.ace}}</td>
        <td>{{getScoreForm.player2.breakrun}}</td>
      </tr>
    </table>
    <div id="score2">
...
import { Player, ScoreForm } from '../types/types';
...
export default class ScoreDisp extends Vue {
  ...
  private drawDiagonalLine() {    
    ...
    if (this.getScoreForm.player1.ball > 0) {
      for (let i=0;i<this.getScoreForm.player1.ball;i++){
        document.querySelectorAll('#score1>span')[i].classList.add('right_up');
      }
    }

    if (this.getScoreForm.player2.ball > 0) {
      for (let i=0;i<this.getScoreForm.player2.ball;i++){
  ...
  private drawCircle() {
    ...
    if (this.getScoreForm.player1.goal > 0) {
      console.log("入ったりょ");
      document.querySelectorAll('#score1>span')[this.getScoreForm.player1.goal-1].classList.add('goal');
    }
    if (this.getScoreForm.player2.goal > 0) {
      document.querySelectorAll('#score2>span')[this.getScoreForm.player2.goal-1].classList.add('goal');
    }
  }
  ...
  get getScoreForm(): ScoreForm {
    return this.$store.getters.getScoreForm;
  }
}
</script>
